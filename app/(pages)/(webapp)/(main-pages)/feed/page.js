'use client';
import { AppNavbar, ArticleCard, SuggestedProfilesModal, useUserStore, useState, useEffect, HomeSkeleton, useRouter, GetCategories, GetFeedPostsApi } from '@/app/ui/imports'
...
export default function Feed() {
    ...
    const fetchPosts = async () => {
        setLoading(true);
        try {
            const { response, jsonResponse, error } = await GetFeedPostsApi(selectedCategory);
            if (error) {
                setPostError(error);
                setLoading(false);
                return;
    ...
    return (
        <div>
            <div className='flex-1'>
                ...
                <p className="text-gray-500 mb-4">Here are today’s most interesting articles from your  <span className='font-semibold'>feed</span></p>
            ...
            <div className="flex flex-col lg:flex-row w-full items-start gap-8">
                {loading ?
                    ...
                    posts?.posts?.length > 0 ?
                        ...
                        :
                        postError == null ?
                            <p className="text-center  w-full text-[#757575] pt-9">No Posts for now...follow more writers</p>
